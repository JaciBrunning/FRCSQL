mod :events, [:districts] do
    ...
    @db.transaction do
        @events.each do |e|
            event = Event.find_or_create(id: e["__key__"]["name"], code: e["event_short"],
            ...
            unless e["district_key"].nil? || e["district_key"]["name"].nil?
                event.district = District[e["district_key"]["name"]]
                event.save
            end
    ...
    @db.transaction do
        @events.each do |e|
            unless e["parent_event"].nil? || e["parent_event"]["name"].nil?
                event = Event[e["__key__"]["name"]]
                parent = Event[e["parent_event"]["name"]]
                event.parent = parent
                event.save
            end
        end
